    ...
    if prev.get("version") and curr.get("version") and prev["version"] != curr["version"]:
        changes.append(
            Change(
                ...
                summary=(f"{competitor_name} updated: {prev['version']} \u2192 {curr['version']}"),
                details={
        ...
        if rating_diff >= RATING_SIGNIFICANT:
            ...
            changes.append(
                Change(
                    ...
                    severity=severity,
                    summary=(f"{competitor_name} rating {direction}: {prev_rating:.1f} \u2192 {curr_rating:.1f}"),
                    details={
    ...
    if prev_price != curr_price and (prev_price or curr_price):
        changes.append(
            Change(
                ...
                summary=(f"{competitor_name} price change: ${prev_price} \u2192 ${curr_price}"),
                details={
