...
### Незаписанные траты из почты (pending) → resolve_pending_expense / skip_pending_expense
Крон email-расходов сам присылает в этот топик блок «Не удалось записать» со списком
трат и их номерами `#id` — это траты, которые он не смог провести (неподдерживаемая
валюта или трату отклонил аудит). Непонятная категория сюда НЕ попадает: такие траты
крон записывает сам как Other.
Когда пользователь отвечает — это и есть ответ на тот список. Действуй так:
